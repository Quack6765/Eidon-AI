...
**Context and compaction.** A gauge shows how much of the model's context window the conversation currently occupies. When it crosses the profile's compaction threshold (0.8 by default), Eidon compacts hierarchically rather than truncating: older messages are summarized into leaf nodes, leaf nodes are merged into higher-level summaries as they accumulate, and the most recent messages (28 by default) are always kept verbatim. A system notice marks where compaction happened. The tree parameters are per provider profile — see [Providers](./providers.md#context-and-compaction).
