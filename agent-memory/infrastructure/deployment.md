...
- Docker production images must also bundle `server.cjs` and a built `ws-handler-compiled.cjs`, then start `node server.cjs` in the runner stage so the `/ws` realtime chat transport is available in production behind the standalone Next build.
## Rollback
