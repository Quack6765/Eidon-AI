...
- **Draft Conversation Cleanup:** `DELETE /api/conversations/[conversationId]?onlyIfEmpty=1` is an idempotent guarded delete used by the chat UI to prune placeholder conversations that never received a first message
...
- **Chat Streaming Events:** `POST /api/conversations/[conversationId]/chat` streams `thinking_delta`, `answer_delta`, `answer_commit`, `system_notice`, `action_start`, `action_complete`, `action_error`, `done`, and `error` SSE events
...
- **Assistant Timeline Persistence:** Assistant prose segments that occur between tool or skill steps are persisted in `message_text_segments` with the same shared sort ordering as `message_actions`, then hydrated into a unified per-message timeline for replay after refresh
