...
- **Settings Guidance:** The settings form manages multiple saved provider profiles, lets the operator mark one as default, applies provider presets for Ollama Cloud, GLM Coding Plan, and Custom OpenAI compatible setups, and shows whether the currently selected model/API mode can produce visible reasoning summaries
