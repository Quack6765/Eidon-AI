...
- **Tool Permissions:** In `Read-Only` mode, Eidon only exposes tools with `annotations.readOnlyHint === true`; unannotated tools are treated as non-read-only and hidden
- **Capability Inventory:** Before each assistant turn, Eidon merges a unified capability inventory into the primary system prompt so the model sees enabled skills, configured MCP servers, and currently executable MCP tools in one place
- **Capability Questions:** Direct "what tools / skills / MCP servers do you have?" questions are answered from Eidon's own runtime inventory instead of relying on the model to describe its environment from memory
...
- **Empty Post-Tool Replies:** If a provider returns an empty assistant answer after tools or skills were used, Eidon adds a corrective system message and retries instead of persisting a silent blank answer immediately
