...
function normalizeReasoningEffort(
  ...
  if (effort === "none") {
    return isOfficialOpenAIEndpoint(settings) && isOpenAIGpt56Model(settings.model)
      ? "none"
      : undefined;
  }

  if (effort === "xhigh" || effort === "max") {
    if (isOfficialOpenAIEndpoint(settings) && isOpenAIGpt56Model(settings.model)) {
      return effort;
    }
...
function isOfficialOpenAIEndpoint(settings: ProviderProfile) {
  const apiBaseUrl = settings.apiBaseUrl.trim().replace(/\/+$/, "").toLowerCase();

  return apiBaseUrl === "https://api.openai.com/v1";
}
...
function buildOpenAIServiceTier(settings: ProviderProfile) {
  if (!isOfficialOpenAIEndpoint(settings)) {
    return {};
  }

  return {
    service_tier: settings.serviceTier ?? "default"
  };
}
...
export async function callProviderText(input: {
  ...
  if (profile.apiMode === "responses") {
    ...
    const request = {
      ...
      reasoning,
      ...buildTemperatureParameter(profile),
      ...buildOpenAIServiceTier(profile)
    };
...
export async function* streamProviderResponse(input: {
  ...
  ChatStreamEvent,
  { answer: string; thinking: string; toolCalls?: ProviderToolCall[]; responseItems?: ProviderResponseItem[]; reasoningSignature?: string; usage: { inputTokens?: number; outputTokens?: number; reasoningTokens?: number; cacheReadTokens?: number; cacheCreationTokens?: number } },
  void
> {
  ...
  if (settings.apiMode === "responses") {
    ...
    const responseCreateParams: Record<string, unknown> = {
      ...
      reasoning,
      ...buildTemperatureParameter(settings),
      ...buildOpenAIServiceTier(settings)
    };
