...
function buildProfile(
  overrides: Partial<{
    ...
    providerPresetId: "ollama_cloud" | "glm_coding_plan" | "openai_official" | "openrouter" | "opencode_go" | "deepseek" | "anthropic_official" | "opencode_go_anthropic" | null;
  }> = {}
) {
  return {
...
describe("settings storage", () => {
  ...
  it("persists the OpenAI processing mode per provider profile", () => {
    ...
    expect(getProviderProfile(profile.id)?.serviceTier).toBe("fast");
  });
  ...
  it("stores max reasoning effort for GPT-5.6 profiles", () => {
    const profile = buildProfile({
      id: "profile_openai",
      apiBaseUrl: "https://api.openai.com/v1",
      model: "gpt-5.6-luna",
    ...
    expect(listProviderProfiles()[0].reasoningEffort).toBe("max");
  });
